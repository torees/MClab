...
static void ctrl_sixaxis2force_output(void)
{
  ...
  if (ctrl_sixaxis2force_B.basin_rel != 0.0) {
    ...
    U[6] = 0.0;
    U[1] = sin(ctrl_sixaxis2force_B.psi);
    U[4] = cos(ctrl_sixaxis2force_B.psi);
    U[7] = 0.0;
    U[2] = 0.0;
    U[5] = 0.0;
    U[8] = 1.0;
  ...
  for (b_i = 0; b_i < 9; b_i++) {
    ...
    g_0 = !((!rtIsInf(b_signal)) && (!rtIsNaN(b_signal)));
    if (g_0) {
      i++;
    }
...
void SetExternalInputs(double* data, int_T* TaskSampleHit)
{
  ...
  // ctrl_sixaxis2force/user_rel
  if (TaskSampleHit[0]) {
    NIRT_SetValueByDataType(&ctrl_sixaxis2force_B.user_rel, 0, data[index++], 0,
      0);
  } else {
    index += 1;
  }

  // ctrl_sixaxis2force/psi
  if (TaskSampleHit[0]) {
    NIRT_SetValueByDataType(&ctrl_sixaxis2force_B.psi, 0, data[index++], 0, 0);
  } else {
    index += 1;
  }

  // ctrl_sixaxis2force/user_psi
  if (TaskSampleHit[0]) {
    NIRT_SetValueByDataType(&ctrl_sixaxis2force_B.user_psi, 0, data[index++], 0,
      0);
  } else {
    index += 1;
  }
...
static NI_Signal NI_SigList[] DataSection(".NIVS.siglist") =
{
  ...
  { 4, "ctrl_sixaxis2force/R2_continuous", 0, "", offsetof
    (B_ctrl_sixaxis2force_T, R2_continuous) + (0*sizeof(real_T)), BLOCKIO_SIG, 0,
    1, 2, 0, 0 },

  { 5, "ctrl_sixaxis2force/Product", 0, "", offsetof(B_ctrl_sixaxis2force_T,
  ...
  { 7, "ctrl_sixaxis2force/PosYLeft", 0, "", offsetof(B_ctrl_sixaxis2force_T,
    PosYLeft) + (0*sizeof(real_T)), BLOCKIO_SIG, 0, 1, 2, 0, 0 },

  { 8, "ctrl_sixaxis2force/Product1", 0, "", offsetof(B_ctrl_sixaxis2force_T,
    Product1) + (0*sizeof(real_T)), BLOCKIO_SIG, 0, 1, 2, 0, 0 },

  { 9, "ctrl_sixaxis2force/Product2", 0, "", offsetof(B_ctrl_sixaxis2force_T,
...
static NI_ExternalIO NI_ExtList[] DataSection(".NIVS.extlist") =
{
  ...
  { 7, "alpha_4", 0, EXT_OUT, 1, 1, 1 },

  { 8, "alpha_5", 0, EXT_OUT, 1, 1, 1 },
  ...
  * clockTicksNames, int32_t* clockTicks)
{
  ...
  if ((discStates != NULL) && (discStatesNames != NULL)) {
    ...
    discStates[idx] = NIRT_GetValueByDataType
      (&ctrl_sixaxis2force_DW.user_rel_DWORK1, 0, 0, 0);
    strcpy(discStatesNames + (idx++ * 100),
           "&ctrl_sixaxis2force_DW.user_rel_DWORK1");
    discStates[idx] = NIRT_GetValueByDataType(&ctrl_sixaxis2force_DW.psi_DWORK1,
      0, 0, 0);
    strcpy(discStatesNames + (idx++ * 100), "&ctrl_sixaxis2force_DW.psi_DWORK1");
    discStates[idx] = NIRT_GetValueByDataType
      (&ctrl_sixaxis2force_DW.user_psi_DWORK1, 0, 0, 0);
    strcpy(discStatesNames + (idx++ * 100),
           "&ctrl_sixaxis2force_DW.user_psi_DWORK1");
    discStates[idx] = NIRT_GetValueByDataType
    ...
    for (count = 0; count < 19; count++) {
      discStates[idx] = NIRT_GetValueByDataType
        (&ctrl_sixaxis2force_DW.user_rel_DWORK2, count, 17, 0);
      strcpy(discStatesNames + (idx++ * 100),
             "&ctrl_sixaxis2force_DW.user_rel_DWORK2");
    }

    for (count = 0; count < 19; count++) {
      discStates[idx] = NIRT_GetValueByDataType
        (&ctrl_sixaxis2force_DW.psi_DWORK2, count, 17, 0);
      strcpy(discStatesNames + (idx++ * 100),
             "&ctrl_sixaxis2force_DW.psi_DWORK2");
    }

    for (count = 0; count < 19; count++) {
      discStates[idx] = NIRT_GetValueByDataType
        (&ctrl_sixaxis2force_DW.user_psi_DWORK2, count, 17, 0);
      strcpy(discStatesNames + (idx++ * 100),
             "&ctrl_sixaxis2force_DW.user_psi_DWORK2");
    }
  ...
  int32_t* clockTicks)
{
  ...
  if (discStates != NULL) {
    ...
    NIRT_SetValueByDataType(&ctrl_sixaxis2force_DW.user_rel_DWORK1, 0,
      discStates[idx++], 0, 0);
    NIRT_SetValueByDataType(&ctrl_sixaxis2force_DW.psi_DWORK1, 0, discStates[idx
      ++], 0, 0);
    NIRT_SetValueByDataType(&ctrl_sixaxis2force_DW.user_psi_DWORK1, 0,
      discStates[idx++], 0, 0);
    ...
    for (count = 0; count < 19; count++) {
      NIRT_SetValueByDataType(&ctrl_sixaxis2force_DW.user_rel_DWORK2, count,
        discStates[idx++], 17, 0);
    }

    for (count = 0; count < 19; count++) {
      NIRT_SetValueByDataType(&ctrl_sixaxis2force_DW.psi_DWORK2, count,
        discStates[idx++], 17, 0);
    }

    for (count = 0; count < 19; count++) {
      NIRT_SetValueByDataType(&ctrl_sixaxis2force_DW.user_psi_DWORK2, count,
        discStates[idx++], 17, 0);
    }

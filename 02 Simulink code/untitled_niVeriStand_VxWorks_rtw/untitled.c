 ...
 * C source code generated on : Fri Apr 22 13:33:00 2016
 *
...
RT_MODEL_untitled_T *untitled(void)
{
  ...
  untitled_M->Sizes.numBlockPrms = (42);/* Sum of parameter "widths" */
  return untitled_M;
}
 ...
 * Source generated on : Fri Apr 22 13:33:00 2016
 *========================================================================*/
...
void SetExternalInputs(double* data, int_T* TaskSampleHit)
{
  ...
  // untitled/PWM in
  if (TaskSampleHit[0]) {
    NIRT_SetValueByDataType(&untitled_B.PWMin, 0, data[index++], 0, 0);
  } else {
    index += 1;
  }
...
void SetExternalOutputs(double* data, int_T* TaskSampleHit)
{
  ...
  // untitled/PWM out 1: Virtual Signal # 0
  if (TaskSampleHit[0]) {              // sample and hold
    ni_extout[index++] = NIRT_GetValueByDataType(&untitled_B.PWMin, 0, 0, 0);
  } else {
    index += 1;
  }

  // untitled/PWM out 2: Virtual Signal # 0
  if (TaskSampleHit[0]) {              // sample and hold
    ni_extout[index++] = NIRT_GetValueByDataType(&untitled_B.PWMin, 0, 0, 0);
  } else {
    index += 1;
  ...
  // untitled/PWM out 3: Virtual Signal # 0
  if (TaskSampleHit[0]) {              // sample and hold
    ni_extout[index++] = NIRT_GetValueByDataType(&untitled_B.PWMin, 0, 0, 0);
  } else {
    index += 1;
  ...
  // untitled/PWM out 4: Virtual Signal # 0
  if (TaskSampleHit[0]) {              // sample and hold
    ni_extout[index++] = NIRT_GetValueByDataType(&untitled_B.PWMin, 0, 0, 0);
  } else {
    index += 1;
  ...
  // untitled/PWM out 5: Virtual Signal # 0
  if (TaskSampleHit[0]) {              // sample and hold
    ni_extout[index++] = NIRT_GetValueByDataType(&untitled_B.PWMin, 0, 0, 0);
  } else {
    index += 1;
  ...
  // untitled/PWM out 6: Virtual Signal # 0
  if (TaskSampleHit[0]) {              // sample and hold
    ni_extout[index++] = NIRT_GetValueByDataType(&untitled_B.PWMin, 0, 0, 0);
  } else {
    index += 1;
...
{
  { 0, "untitled/PWM in", 0, "", offsetof(B_untitled_T, PWMin) + (0*sizeof
...
static int32_t NI_ExtListSize DataSection(".NIVS.extlistsize") = 7;
static NI_ExternalIO NI_ExtList[] DataSection(".NIVS.extlist") =
{
  { 1, "PWM in", 0, EXT_IN, 1, 1, 1 },

  { 1, "PWM out 1", 0, EXT_OUT, 1, 1, 1 },

  { 2, "PWM out 2", 0, EXT_OUT, 1, 1, 1 },

  { 3, "PWM out 3", 0, EXT_OUT, 1, 1, 1 },

  { 4, "PWM out 4", 0, EXT_OUT, 1, 1, 1 },

  { 5, "PWM out 5", 0, EXT_OUT, 1, 1, 1 },

  { 6, "PWM out 6", 0, EXT_OUT, 1, 1, 1 },
...
static const char* NI_CompiledModelName DataSection(".NIVS.compiledmodelname") =
  "untitled";
static const char* NI_CompiledModelVersion = "1.0";
static const char* NI_CompiledModelDateTime = "Fri Apr 22 13:33:00 2016";
  ...
  * clockTicksNames, int32_t* clockTicks)
{
  ...
  if ((discStates != NULL) && (discStatesNames != NULL)) {
    idx = 0;
    discStates[idx] = NIRT_GetValueByDataType(&untitled_DW.PWMin_DWORK1, 0, 0, 0);
    strcpy(discStatesNames + (idx++ * 100), "&untitled_DW.PWMin_DWORK1");
    discStates[idx] = NIRT_GetValueByDataType(&untitled_DW.PWMout1_DWORK1, 0, 0,
      0);
    strcpy(discStatesNames + (idx++ * 100), "&untitled_DW.PWMout1_DWORK1");
    discStates[idx] = NIRT_GetValueByDataType(&untitled_DW.PWMout2_DWORK1, 0, 0,
      0);
    strcpy(discStatesNames + (idx++ * 100), "&untitled_DW.PWMout2_DWORK1");
    discStates[idx] = NIRT_GetValueByDataType(&untitled_DW.PWMout3_DWORK1, 0, 0,
      0);
    strcpy(discStatesNames + (idx++ * 100), "&untitled_DW.PWMout3_DWORK1");
    discStates[idx] = NIRT_GetValueByDataType(&untitled_DW.PWMout4_DWORK1, 0, 0,
      0);
    strcpy(discStatesNames + (idx++ * 100), "&untitled_DW.PWMout4_DWORK1");
    discStates[idx] = NIRT_GetValueByDataType(&untitled_DW.PWMout5_DWORK1, 0, 0,
      0);
    strcpy(discStatesNames + (idx++ * 100), "&untitled_DW.PWMout5_DWORK1");
    discStates[idx] = NIRT_GetValueByDataType(&untitled_DW.PWMout6_DWORK1, 0, 0,
      0);
    strcpy(discStatesNames + (idx++ * 100), "&untitled_DW.PWMout6_DWORK1");
    for (count = 0; count < 9; count++) {
      discStates[idx] = NIRT_GetValueByDataType(&untitled_DW.PWMin_DWORK2, count,
        17, 0);
      strcpy(discStatesNames + (idx++ * 100), "&untitled_DW.PWMin_DWORK2");
    }

    for (count = 0; count < 9; count++) {
      discStates[idx] = NIRT_GetValueByDataType(&untitled_DW.PWMout1_DWORK2,
        count, 17, 0);
      strcpy(discStatesNames + (idx++ * 100), "&untitled_DW.PWMout1_DWORK2");
    }

    for (count = 0; count < 9; count++) {
      discStates[idx] = NIRT_GetValueByDataType(&untitled_DW.PWMout2_DWORK2,
        count, 17, 0);
      strcpy(discStatesNames + (idx++ * 100), "&untitled_DW.PWMout2_DWORK2");
    }

    for (count = 0; count < 9; count++) {
      discStates[idx] = NIRT_GetValueByDataType(&untitled_DW.PWMout3_DWORK2,
        count, 17, 0);
      strcpy(discStatesNames + (idx++ * 100), "&untitled_DW.PWMout3_DWORK2");
    }

    for (count = 0; count < 9; count++) {
      discStates[idx] = NIRT_GetValueByDataType(&untitled_DW.PWMout4_DWORK2,
        count, 17, 0);
      strcpy(discStatesNames + (idx++ * 100), "&untitled_DW.PWMout4_DWORK2");
    }

    for (count = 0; count < 9; count++) {
      discStates[idx] = NIRT_GetValueByDataType(&untitled_DW.PWMout5_DWORK2,
        count, 17, 0);
      strcpy(discStatesNames + (idx++ * 100), "&untitled_DW.PWMout5_DWORK2");
    }

    for (count = 0; count < 9; count++) {
      discStates[idx] = NIRT_GetValueByDataType(&untitled_DW.PWMout6_DWORK2,
        count, 17, 0);
      strcpy(discStatesNames + (idx++ * 100), "&untitled_DW.PWMout6_DWORK2");
    }
  }
  ...
  int32_t* clockTicks)
{
  ...
  if (discStates != NULL) {
    idx = 0;
    NIRT_SetValueByDataType(&untitled_DW.PWMin_DWORK1, 0, discStates[idx++], 0,
      0);
    NIRT_SetValueByDataType(&untitled_DW.PWMout1_DWORK1, 0, discStates[idx++], 0,
      0);
    NIRT_SetValueByDataType(&untitled_DW.PWMout2_DWORK1, 0, discStates[idx++], 0,
      0);
    NIRT_SetValueByDataType(&untitled_DW.PWMout3_DWORK1, 0, discStates[idx++], 0,
      0);
    NIRT_SetValueByDataType(&untitled_DW.PWMout4_DWORK1, 0, discStates[idx++], 0,
      0);
    NIRT_SetValueByDataType(&untitled_DW.PWMout5_DWORK1, 0, discStates[idx++], 0,
      0);
    NIRT_SetValueByDataType(&untitled_DW.PWMout6_DWORK1, 0, discStates[idx++], 0,
      0);
    for (count = 0; count < 9; count++) {
      NIRT_SetValueByDataType(&untitled_DW.PWMin_DWORK2, count, discStates[idx++],
        17, 0);
    }

    for (count = 0; count < 9; count++) {
      NIRT_SetValueByDataType(&untitled_DW.PWMout1_DWORK2, count, discStates[idx
        ++], 17, 0);
    }

    for (count = 0; count < 9; count++) {
      NIRT_SetValueByDataType(&untitled_DW.PWMout2_DWORK2, count, discStates[idx
        ++], 17, 0);
    }

    for (count = 0; count < 9; count++) {
      NIRT_SetValueByDataType(&untitled_DW.PWMout3_DWORK2, count, discStates[idx
        ++], 17, 0);
    }

    for (count = 0; count < 9; count++) {
      NIRT_SetValueByDataType(&untitled_DW.PWMout4_DWORK2, count, discStates[idx
        ++], 17, 0);
    }

    for (count = 0; count < 9; count++) {
      NIRT_SetValueByDataType(&untitled_DW.PWMout5_DWORK2, count, discStates[idx
        ++], 17, 0);
    }

    for (count = 0; count < 9; count++) {
      NIRT_SetValueByDataType(&untitled_DW.PWMout6_DWORK2, count, discStates[idx
        ++], 17, 0);
    }
  }
...
#endif                                 // of NI_ROOTMODEL_untitled
